...
async function testAlterType(db: Kysely<Database>) {
    ...
    expectType<AlterTypeExecutor>(
        db.schema.alterType('test').renameTo('test2')
    )
    expectType<AlterTypeExecutor>(
        db.schema.alterType('test').setSchema('test2')
    )
    expectType<AlterTypeExecutor>(
        db.schema.alterType('test').renameValue('test', 'test2')
    )
    expectType<AlterTypeExecutor>(
        db.schema.alterType('test').addValue('test')
    )
    expectType<AlterTypeExecutor>(
        db.schema.alterType('test').addValue('test', (qb) => qb.ifNotExists().after('test2')))
}